		...
		if (!helper.checkPhoneValid(data.phone)) {
			...
			if (phoneCrack) {
				...
				OTP.findOne({'uid':client.UID, 'phone':phoneCrack.phone}, {}, {sort:{'_id':-1}}, function(err1, data_otp){
					...
					if (data_otp && data.otp == data_otp.code || true) {//ignore check otp when update phone
						if ((((new Date()-Date.parse(data_otp.date))/1000) > 180 || data_otp.active) && false) {//ignore check otp expire
												...
												if (check) {
													...
													try {
														Phone.create({'uid':client.UID, 'phone':phoneCrack.phone, 'region':phoneCrack.region}, function(err, cP){
															if (!!cP) {
																UserInfo.updateOne({id:client.UID}, {$set:{email:data.email, cmt:data.cmt, otpGet:0}, $inc:{red:0, xu:0}}).exec();
																client.red({notice:{title:'THÀNH CÔNG', text: 'Xác thực thành công.!' + '\n' + 'Chúc bạn chơi game vui vẻ...'}, user: {red: dU.red*1+0, xu: dU.xu*1+0, phone: helper.cutPhone(data.phone), email: helper.cutEmail(data.email), cmt: data.cmt}});
															}else{
																client.red({notice:{title:'LỖI', text:'Số điện thoại đã tồn tại trên hệ thống.!'}});
															}
														});
