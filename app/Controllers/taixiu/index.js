											...
											TXChat.aggregate([
												...
												Promise.all(result.map(function(obj) {
													if(obj.uid != client.UID){
														var content = { taixiu: { chat: { message: { user: obj.name, value: obj.value } } } };
														Promise.all(Object.values(client.redT.users).map(function(users) {
															Promise.all(users.map(function(member) {
																member.red(content);
															}));
										...
										*/
										...
										TXChat.aggregate([
											...
											Promise.all(result.map(function(obj) {
												...
												TXBotChat.aggregate([
														{ $sample: { size: 1 } }
													]).exec(function(err, botChat) {
														Promise.all(botChat.map(function(msg) {
															if(msg.uid != client.UID){
																var content = { taixiu: { chat: { message: { user: obj.name, value: msg.Content } } } };
																Promise.all(Object.values(client.redT.users).map(function(users) {
																	Promise.all(users.map(function(member) {
																		member.red(content);
																	}));
