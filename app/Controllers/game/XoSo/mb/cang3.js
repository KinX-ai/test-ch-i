		...
		if (diem > 0 && diem < 1000000) {
			...
			if(timeCL > 0){
				...
				if (res.length === 0) {
					...
					client.red({XoSo:{notice:'1 Vé cược tối đa 8 Số...'}});
				}else{
					...
					UserInfo.findOne({id:client.UID}, 'red', function(err, users){
						if (!!users && users.red >= tongTien) {
							...
							xsmb_cuoc.create({name:client.profile.name, date:stringTime, type:'3cang', so:res, diem:diem, cuoc:tongTien, time:new Date()});
