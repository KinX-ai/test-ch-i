			...
			Zeus_user.aggregate([
				...
				Zeus_user.aggregate([
					...
					if (result.length) {
						Promise.all(result.map(function(obj){
							return new Promise(function(resolve, reject) {
								UserInfo.findOne({'id': obj.uid}, 'name', function(error, user){
									...
									if (!!user) {
										obj['name'] = user.name;
										Zeus_red.findOne({'name': user.name}, 'bet time', {sort:{'_id': -1}}, function(error, result2){
											if (!!result2) {
												obj['t'] = result2.time;
												obj['b'] = result2.bet;
											}
						...
						.then(function(data){
							client.red({zeus:{get_top:{data:data, page:page, kmess:kmess, total:countFind[0].total}}});
						})
					}else{
						client.red({zeus:{get_top:{data:[], page:1, kmess:9, total:0}}});
					}
				});
			});
		}
